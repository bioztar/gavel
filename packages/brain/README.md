...
| **floorHog** | Over `floorShareThreshold` of the recent window, with others quiet | Thanks them, recaps, and hands the floor to someone by name — at their next pause (`handover: soft`, the default) or cutting in (`hard`). Discussion topics only |
