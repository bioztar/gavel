...
| **floorHog** | Over `floorShareThreshold` of the recent window, with others quiet | Thanks them, recaps, and hands the floor to someone by name — at their next pause past `softHandoverSeconds`, cutting in once they are past `hardHandoverSeconds` too. Discussion topics only |
