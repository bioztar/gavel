...
Field for field the same models as ears-discord/src/ears/frames.py — the brain must not
be able to tell which surface is on the other end. `discord_id` is the contract's generic
participant-id field; on Meet it carries a Meet participant id, and the name stays.
